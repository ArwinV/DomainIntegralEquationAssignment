...
E_bessel = domain_integral_equation(simparams)

# Show the calculated E field
show_plane(np.absolute(E_bessel), step_size, title='E field (Bessel) calculated with algorithm for 'r'$\theta_i$ = %i' %theta_i)
...
# Compute E-field using domain_integral_equation
E_plane = domain_integral_equation(simparams)

# Show the calculated E field
show_plane(np.absolute(E_plane), step_size, title='E field (plane) calculated with algorithm for 'r'$\theta_i$ = %i' %theta_i)
...
show_plane(np.absolute(E_fieldval), step_size, title='E field of analytical solution for 'r'$\theta_i$ = %i' %theta_i)

# Calculate difference between implementation and validation
E_error = np.absolute(E_bessel - E_fieldval)

# Plot the error
show_plane(E_error, step_size, title="Error between analytical solution and algorithm")

# Plot the incident plane waves
show_plane(np.real(E_inval), step_size, title='Incident field (analytical) for 'r'$\theta_i$ = %i' %theta_i)

mu0 = np.pi*4e-7
epsilon0 = 8.854187812813e-12
E_0 = np.sqrt(mu0/epsilon0) # Amplitude of incident wave
E_incident_bessel = create_planewave(simulation_size, step_size, E_0, wavelength, input_angle,1,'bessel')
show_plane(np.real(E_incident_bessel), step_size, title='Incident field (algorithm, bessel function) for 'r'$\theta_i$ = %i' %theta_i)

E_incident_plane = create_planewave(simulation_size, step_size, E_0, wavelength, input_angle,1,'plane')
show_plane(np.real(E_incident_plane), step_size, title='Incident field (algorithm, plane wave) for 'r'$\theta_i$ = %i' %theta_i)